...
def approve_student_view(request,pk):
    studentSalary=forms.StudentSalaryForm()
    if request.method=='POST':
        studentSalary=forms.StudentSalaryForm(request.POST)
        if studentSalary.is_valid():
            student=SMODEL.Student.objects.get(id=pk)
            student.salary=studentSalary.cleaned_data['salary']
            student.status=True
            student.save()
        else:
            print("form is invalid")
        return HttpResponseRedirect('/admin-view-pending-student')
    return render(request,'quiz/salary_form_student.html',{'studentSalary':studentSalary})
...
@login_required(login_url='adminlogin')
def admin_view_student_salary_view(request):
    students= SMODEL.Student.objects.all().filter(status=True)
    return render(request,'quiz/admin_view_student_salary.html',{'students':students})
